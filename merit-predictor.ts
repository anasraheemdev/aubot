...
// Create a type for the keys of the closingMerits object
type ProgramKey = keyof typeof closingMerits;

export function calculateAggregate(ssc_percentage: number, hssc_percentage: number, test_score: number): number {
...
export function predictAdmissionChance(aggregate: number, program: string): string {
    const formattedProgramKey = `BS${program.replace(/\s+/g, '').toUpperCase()}`;

    // Find the key in a type-safe way
    const programKey = Object.keys(closingMerits).find(k => k.toUpperCase() === formattedProgramKey) as ProgramKey | undefined;
